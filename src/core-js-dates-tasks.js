...
function getNextFriday(date) {
  ...
  const addDaysNumber = todayDay < 5 ? 5 - todayDay : 12 - todayDay;
  const nextFridayDate = new Date(date.setDate(date.getDate() + addDaysNumber));
...
function formatDate(date) {
  ...
  const formattedDate = `${month}/${day}/${year}, ${hour}:${minute.toString().padStart(2, '0')}:${second.toString().padStart(2, '0')} ${ampm}`;
...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfYearDayOfWeek = firstDayOfYear.getDay() || 7;
  const firstThursdayOfYear =
    firstDayOfYear.getDate() + (4 - firstDayOfYearDayOfWeek);
  const firstMondayOfYear = firstThursdayOfYear - 3;

  const startDayOfYear = new Date(date.getFullYear(), 0, firstMondayOfYear);

  const diff = date - startDayOfYear;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    if (cycleDay % (countWorkDays + countOffDays) < countWorkDays) {
      schedule.push(formattedDate(currentDate));
    }
